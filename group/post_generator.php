...
$n = $_POST['times'];
$id = $_POST['user'];
$res= $connect->query("SELECT * FROM groups WHERE id_2 = '".$id."' ");
...
while ($row = $result->fetch_assoc()){
	...
	if ($row['page'] == $id && $row['id'] && $row['id'] < $last){
		$res= $connect->query("SELECT * FROM groups WHERE id_2 = '".$row['author_id']."' ");
		...
		$ava = 		'<img src="'.$data['avatar'].'" id="post_av" >';
	  	$ava2 = 	'';
	  	$style = 	'';
	  	$like_icon= '<h5 id="like_label" class="lll'.$row['id'].'">Нравится<img src="/img/like_icon1.png" onclick="like('.$row['id'].',0);" id="like"></h5>';
		if ($text !=''){
			$ava = ''; 
			...
			<div class="row">
				...
				<div class="col-sm-8 post">
					<div class="cointainer-fluid" style="padding: 0;margin: 0;">
						...
						<div class="row">
							'.$ava.'
								<img src="'.$img.'" id="post_bg" class="postimg" style="width:100%;">
